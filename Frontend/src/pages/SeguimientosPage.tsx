...
export const SeguimientosPage = () => {
  ...
  useEffect(() => {
    const cargar = async () => {
      setLoading(true);
      const [segs, conts] = await Promise.all([getSeguimientos(), getContactos()]);
  ...
  const handleCompletar = async (id: number) => {
    const actualizado = await completarSeguimiento(id);
  ...
  const handleCrear = async () => {
    ...
    setSaving(true);
    try {
      const nuevo = await createSeguimiento({ tarea: formData.tarea, fecha: formData.fecha, completado: false, contacto });
    ...
    <div className="space-y-6 animate-fade-in p-2 md:p-6">
              ...
              return (
                <Card key={seg.id} className={`flex items-center gap-4 transition-all ${seg.completado ? 'opacity-60' : ''}`}>
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    {estado === 'hoy' && <Badge variant="warning">{t('seguimientos.badge.today')}</Badge>}
                    {estado === 'completada' && <Badge variant="success">{t('seguimientos.badge.done')}</Badge>}
                    <button className="p-1.5 rounded-lg hover:bg-red-50 text-red-400 transition-colors" onClick={() => { void handleEliminar(seg.id); }}>
      ...
      <Modal isOpen={modalOpen} title={t('seguimientos.new')} onClose={() => { setModalOpen(false); }}>
        <div className="space-y-4">
          <Input id="tarea-desc" label={t('seguimientos.taskDescription')} placeholder={t('seguimientos.taskPlaceholder')} type="text" value={formData.tarea} onChange={e => { setFormData(p => ({ ...p, tarea: e.target.value })); }} />
          <Input id="tarea-fecha" label={t('seguimientos.dueDate')} type="date" value={formData.fecha} onChange={e => { setFormData(p => ({ ...p, fecha: e.target.value })); }} />
          <div>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <Button className="flex-1" isLoading={saving} onClick={() => { void handleCrear(); }}>{t('seguimientos.createTask')}</Button>
          </div>
        </div>
      </Modal>
    </div>
